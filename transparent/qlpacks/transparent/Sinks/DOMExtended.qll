...
class DomMethodCallNodeExtended extends DataFlow::MethodCallNode {
    ...
    predicate isFixedSink(DataFlow::Node payloadSink) {
        interpretsArgumentAsHtml(payloadSink)
    }
    ...
    predicate interpretsArgumentAsHtml(DataFlow::Node arg) {
        ...
        exists(int argPos, string name | arg = this.getArgument(argPos) and name = this.getMethodName() | 
            // Node
...
class DomPropertyWriteExtended extends DataFlow::Node instanceof DataFlow::PropWrite {
    DomPropertyWriteExtended() { 
        ...
        // Heuristic: if there's a property access statement and its `innerHTML`, then the base is a sink
        // TODO: expand this to not just `innerHTML`
        or exists (DataFlow::PropRef p | p.getPropertyName() = "innerHTML" | 
    ...
    predicate interpretsValueAsHtml() {
        isDomPropInterpretedAsHtml(super.getPropertyName())
    }

    predicate interpretsValueAsUrl() {
        isDomPropInterpretedAsUrl(super.getPropertyName())
    }
...
predicate isDomPropInterpretedAsHtml(string propName) {
    propName = "innerHTML" or 
    propName = "outerHTML"
    // below are new! (these technically aren't HTML, since they use `Element` object)
    // propName = "body"    // assignment to `body` must use `innerHTML` anyway
}

predicate isDomPropInterpretedAsUrl(string propName) {
    // src, href, action, formaction, data
    propName = DOM::getAPropertyNameInterpretedAsJavaScriptUrl() or
    // below are new!
    propName = "srcdoc" or
    propName = "xlink:href" or
    propName = "code" or     // Chrome-specific
    propName = "data"        // Firefox-specific
}